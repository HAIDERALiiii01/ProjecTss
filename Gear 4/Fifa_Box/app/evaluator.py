...
ANSWER_GREEN = 4.5
ANSWER_AMBER = 4.0
FAITHFULNESS_GREEN = 4.5
FAITHFULNESS_AMBER = 4.0
...
def get_color(value: float, metric_type: str) -> str:
    ...
    if metric_type == "mrr":
        ...
        if value >= FAITHFULNESS_GREEN:
            return "green"
        elif value >= FAITHFULNESS_AMBER:
            return "orange"
        else:
            return "red"
    return "black"
...
def run_answer_evaluation(progress=gr.Progress()):
    ...
    for test, answer_result, faithfulness_result, prog_value in evaluate_all_answers():
        count += 1
        total_accuracy += answer_result.accuracy
        total_completeness += answer_result.completeness
        total_relevance += answer_result.relevance
        total_faithfulness += faithfulness_result.faithfulness

        category_accuracy[test.category].append(answer_result.accuracy)
    ...
    <div style="padding: 0;">
        ...
        {format_metric_html("Faithfulness", avg_faithfulness, "faithfulness", score_format=True)}
        <div style="margin-top: 20px; padding: 10px; background-color: #d4edda; border-radius: 5px; text-align: center; border: 1px solid #c3e6cb;">
            <span style="font-size: 14px; color: #155724; font-weight: bold;">✓ Evaluation Complete: {count} tests</span>
        </div>
    </div>
